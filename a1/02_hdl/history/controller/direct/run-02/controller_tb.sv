...
module controller_tb;
  ...
  task automatic apply_and_check(
    ...
    begin
      // Coverage summary:
      // - one or more legal cases for every specified controller class
      // - all ALUSelect classes across I-type and R-type instructions
      // - sample examples from the specification
      // - strict legality checks for invalid opcode/funct3/funct7/shift-immediate forms
      // - immediate-fragmented encodings through S/B/J/U constructors
      // - corner cases for zero, all ones, sign-boundary immediates, and shift amount 0/31
      InstrD = instr;
      #1;
  ...
  initial begin
    ...
    // Loads: all legal and all illegal funct3 values.
    ...
    apply_and_check("illegal load3",  mk_itype(12'h111, 5'd1, 3'b011, 5'd2, 7'b0000011));
    apply_and_check("illegal load6",  mk_itype(12'h222, 5'd1, 3'b110, 5'd2, 7'b0000011));
    apply_and_check("illegal load7",  mk_itype(12'h333, 5'd1, 3'b111, 5'd2, 7'b0000011));

    // I-type ALU: all operation classes, signed-compare classes, shift masking boundaries.
    apply_and_check("addi zero",      mk_itype(12'h000, 5'd1, 3'b000, 5'd2, 7'b0010011));
    apply_and_check("addi -1",        mk_itype(12'hfff, 5'd1, 3'b000, 5'd2, 7'b0010011));
    apply_and_check("addi min12",     mk_itype(12'h800, 5'd1, 3'b000, 5'd2, 7'b0010011));
    apply_and_check("addi max12",     mk_itype(12'h7ff, 5'd1, 3'b000, 5'd2, 7'b0010011));
    ...
    apply_and_check("slli sh0",       instr_tmp);
    shamt5 = 5'd31;
    instr_tmp = mk_itype({7'b0000000, shamt5}, 5'd1, 3'b001, 5'd2, 7'b0010011);
    apply_and_check("slli sh31",      instr_tmp);

    shamt5 = 5'd0;
    instr_tmp = mk_itype({7'b0000000, shamt5}, 5'd1, 3'b101, 5'd2, 7'b0010011);
    apply_and_check("srli sh0",       instr_tmp);
    shamt5 = 5'd31;
    instr_tmp = mk_itype({7'b0000000, shamt5}, 5'd1, 3'b101, 5'd2, 7'b0010011);
    apply_and_check("srli sh31",      instr_tmp);

    shamt5 = 5'd0;
    instr_tmp = mk_itype({7'b0100000, shamt5}, 5'd1, 3'b101, 5'd2, 7'b0010011);
    apply_and_check("srai sh0",       instr_tmp);
    shamt5 = 5'd31;
    instr_tmp = mk_itype({7'b0100000, shamt5}, 5'd1, 3'b101, 5'd2, 7'b0010011);
    apply_and_check("srai sh31",      instr_tmp);

    shamt5 = 5'd1;
    instr_tmp = mk_itype({7'b0100000, shamt5}, 5'd1, 3'b001, 5'd2, 7'b0010011);
    apply_and_check("illegal slli",   instr_tmp);
    shamt5 = 5'd2;
    instr_tmp = mk_itype({7'b0010000, shamt5}, 5'd1, 3'b101, 5'd2, 7'b0010011);
    apply_and_check("illegal shifti", instr_tmp);

    // U-type instructions, including zero and all ones upper immediates.
    apply_and_check("auipc zero",     mk_utype(20'h00000, 5'd1, 7'b0010111));
    apply_and_check("auipc ones",     mk_utype(20'hfffff, 5'd31, 7'b0010111));
    apply_and_check("lui zero",       mk_utype(20'h00000, 5'd2, 7'b0110111));
    apply_and_check("lui ones",       mk_utype(20'hfffff, 5'd30, 7'b0110111));

    // Stores: legal and illegal funct3 values; fragmented S-immediates.
    ...
    apply_and_check("illegal st3",    mk_stype(12'h123, 5'd1, 5'd2, 3'b011, 7'b0100011));
    apply_and_check("illegal st4",    mk_stype(12'h456, 5'd1, 5'd2, 3'b100, 7'b0100011));
    apply_and_check("illegal st7",    mk_stype(12'h789, 5'd1, 5'd2, 3'b111, 7'b0100011));

    // R-type ALU: every class, both legal funct7 variants where defined, and illegal funct7.
    ...
    apply_and_check("illegal r add",  mk_rtype(7'b0000001, 5'd